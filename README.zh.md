...
- Node.js 与 DeepSeek Harness。本应用**两者都不捆绑**——磁盘映像只有一兆左右。自行安装：
  ```sh
  npm install -g @deepseek-ai/dsh
  ```
  也可以在找不到时让应用代为下载一份（见[托管运行时](#托管运行时)；这是可选的，而且你
  自己装的运行时永远优先）。
...
什么都不捆绑，也不会背着你下载任何东西。当找不到 Node.js 或 dsh 时，应用会**询问**是否
...
最低版本是**查找条件**而不是查完再检查，正是为了不让一个过旧的 node 遮蔽掉可用的那个。
...
Node 的 tarball 会先与**固定在启动器脚本里的校验和**比对，通过后才解压。如果校验和跟
...
的应用。其余细节见 [SECURITY.md](SECURITY.md#updates)。
...
这次查询是匿名的，因此会占用 GitHub 每小时 60 次、**按 IP 计算**的 API 额度。在共享出口
